...
AUFileAudioSink::AUFileAudioSink(const std::string& filename, int out_count) : _count(out_count)
{
	_sfinfo.samplerate = 8000;
	_sfinfo.channels = 1;
//	_sfinfo.format = SF_FORMAT_AU | SF_FORMAT_FLOAT;
	_sfinfo.format = SF_FORMAT_AU | SF_FORMAT_PCM_16;
...
void AUFileAudioSink::playSamples(Ipp32f* samples)
{
	static int phase = 0;
